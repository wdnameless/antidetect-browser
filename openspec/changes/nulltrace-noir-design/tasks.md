...
- [x] 1.1 Neutralise the remaining hue in `:root`: `--danger`, `--danger-bg`, `--ok`,
...
- [x] 1.2 Extend the token layer with what the new components need and what pages
...
- [x] 2.4 Sidebar footer → ShardX shape: an AUTOMATION API panel with a live status
...
- [x] 2.5 Keep sidebar collapse working exactly as now (`sidebar.collapsed`,
      Ctrl/Cmd+B, `sidebarLogic.ts`) — grouping must not break it.
- [x] 2.6 i18n for every new visible string, **both `en` and `ru`**.

## 3. Frameless window

- [x] 3.1 `electron/main.ts` `BrowserWindow` (`:178`) becomes frameless and the
      application menu is removed.
- [x] 3.2 The app supplies its own draggable header region and
      minimise / maximise / close controls. Verify the window can still be moved,
      minimised, maximised and closed.
- [x] 3.3 **Preserve tray behaviour and close-to-tray** (`main.ts:189-196`). Verify
      hide-to-tray, tray restore and quit all still work — an unmovable or
      unclosable window is a hard failure, not a cosmetic one.
- [x] 3.4 The controls are Electron-only. In a browser-served client they must not
      render — a button that does nothing is worse than no button.

## 4. Frames off (R61/R62)

- [x] 4.1 Remove box borders and elevation from containers: `.table-container`
      (`styles.css:574`), `.panel` (`:1198`), `.modal-card` (`:1144`),
      preflight/proxy boxes (`:786,847,1086,1106`), `.error-banner` (`:1264`).
- [x] 4.2 Remove borders from controls: `.btn` (`:428`), `.btn-icon` (`:482`),
      ...
      assuming.
- [x] 4.3 Flatten chips/pills: `.badge` (`:641,656`), `.platform-tag` (`:673`),
...
- [x] 4.4 Row action buttons (`Profiles.tsx:1515,1537,1546,1559`) lose their boxes.
- [x] 4.5 **Keep** the hairline dividers: table row separators, section dividers,
      modal header/footer, sidebar footer edge. This is the line between R61 and R62.
- [x] 4.6 Semantic states without colour: running/closed, ok/warn/failed must remain
      distinguishable by weight, shape or icon. Verify by looking at them together.
- [x] 4.7 Operator data colours (profile/tag pickers) are untouched — not chrome.

## 5. Page sweep (worst-first)

- [x] 5.1 `FlowCanvas.tsx` — largest; also bakes raw hex into inline SVG edge markers
      (`:1285-1307,1349,1395`).
- [x] 5.2 `Profiles.tsx` — heaviest inline usage.
- [x] 5.3 `Email.tsx`, `Calendar.tsx`, `Diagnostics.tsx`, `FleetPanel.tsx`
      (`STATUS_COLORS` hex map at `:21-27`).
- [x] 5.4 Remaining pages onto tokens; shared primitives replace inline re-implementations.
- [x] 5.5 Empty states consolidated onto one shared primitive (currently several
