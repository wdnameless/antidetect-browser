...
- [x] 1.1 New kernel-download module: fetch the platform-correct archive from the
...
- [x] 1.2 Pin all three platform artefacts with their published digests:
...
- [x] 1.3 **Fail closed.** A digest mismatch, a truncated body, or a missing release
...
- [x] 1.4 `kernelUpdate.ts` keeps its version check; the new module owns acquisition.
      Do not duplicate the upstream-release lookup.
- [x] 1.5 First-run behaviour: when the kernel is absent, the app acquires it and
      shows progress; with no network it must say so plainly instead of failing to
      start silently.
- [x] 1.6 Remove the kernel from `extraResources` once acquisition works, so the
      artefact stops carrying 425 MB. Verify the build still resolves a kernel.

## 2. Single-file artefacts

- [x] 2.1 Windows: add the `portable` target and remove `nsis`, so no installer is
      produced. Keep `appId` unchanged so existing installs can move across.
- [x] 2.2 Linux: add the `AppImage` target with the correct icon and category.
- [x] 2.3 macOS: add the `dmg` target for arm64 (per the earlier platform decision),
      and document the quarantine step in the README — we have no Apple account, so
      it is unsigned.
- [x] 2.4 `artifactName` must make each platform's artefact self-describing
...
- [x] 3.1 `resolveDataDir` (`config.ts:37`) gains a `portable` mode: data beside the
...
- [x] 3.2 First-run choice: on a portable launch with no recorded choice, ask whether
...
- [x] 3.3 Verify portability end-to-end: with portable mode chosen, the data
      directory sits next to the executable, and moving that folder to another path
      keeps profiles and settings reachable.
- [x] 3.4 A non-portable build (the ordinary desktop app) MUST NOT be asked this
      question — the prompt belongs to the portable launch path only.

## 4. CI (all three platforms)

- [x] 4.1 Extend the release job to build every platform on its own runner:
      `windows-latest` (portable exe), `ubuntu-latest` (AppImage),
      `macos-14` (arm64 dmg). Each uploads its artefact.
- [x] 4.2 The job must not require an Apple certificate or signing secrets —
      the macOS artefact ships unsigned, and the workflow must not fail without
      credentials it cannot have.
- [x] 4.3 Run the existing typecheck/build/test gates on the Linux and macOS runners
      too, so a platform-specific break is caught rather than shipped.
- [x] 4.4 Deliver as a PR from `fix/ci-pipeline` to `main` (the user reviews and
      merges). Do not push to `main` directly.

## 5. Verification

- [x] 5.1 Build the Windows portable artefact locally and **run it**: confirm it
      launches, acquires the kernel, and serves the UI — a build that merely
      succeeds is not evidence.
- [x] 5.2 Confirm no installer artefact is produced.
- [x] 5.3 Full suite green, typecheck clean, CHANGELOG entry, README gains a
      "Portable" section covering all three platforms and the macOS quarantine step.
- [x] 5.4 Record the deferred item: the monochrome noir redesign (R51i) runs after
      this program.
