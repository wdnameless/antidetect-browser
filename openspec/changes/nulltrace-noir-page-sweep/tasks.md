...
- [ ] 1.1 Extend `tests/unit/noirTokens.test.ts` from `styles.css` only to **every
...
- [ ] 1.2 The same guard must fail on an orphan reference: no `var(--x, #hex)` anywhere
      in renderer sources. Email.tsx currently has 20 of these and must reach zero.
- [ ] 1.3 The guard must NOT flag operator data colours. Those live in the colour
...
- [ ] 2.1 Neutral greys → token references. `#a1a1aa` → `--text-secondary`,
...
- [ ] 2.2 **Validation state** (`#22c55e` / `#ef4444` at ~1196-1197) — this drives a
...
- [ ] 2.3 **Live-run indicator** (`#3b82f6` at ~1484, `#22c55e`) — same rule: this is
      `isRunning ? … : …`, keep the branch, express the two states without hue.
- [ ] 2.4 **SVG edge markers** (`stroke="#3b82f6"`, `#ef4444`, `#ffffff`,
...
- [ ] 2.5 Node/edge/palette colours that encode the node TYPE (a colour per node kind)
...
- [ ] 3.1 `pages/Email.tsx` — 51 literals AND 20 orphan `var(--x, #hex)`. Worst
      offender after FlowCanvas; the orphan dialect must reach zero.
- [ ] 3.2 `components/FleetPanel.tsx` — 18 literals; its `STATUS_COLORS` map already
      moved to tokens, so verify these are the remaining inline ones.
- [ ] 3.3 `pages/Profiles.tsx` (8), `pages/Calendar.tsx` (8) — **but preserve the
      operator's profile/tag colour choices**; only chrome changes (R76i).
- [ ] 3.4 `pages/Diagnostics.tsx` (5 + 1 orphan) — its `statusColor()` function returns
      hard hex and must become token-based.
- [ ] 3.5 `pages/SecuritySettings.tsx`, `pages/SyncSettings.tsx`, `pages/Extensions.tsx`
      — small counts, including two orphans.

## 4. The two reported defects

- [ ] 4.1 **Duplicate titles**: remove the page-level `h2` in `Proxies.tsx:156-170` and
      `Extensions.tsx:104-114` — the shell now renders the title. One title per page.
- [ ] 4.2 **Empty states**: extract one primitive (`components/EmptyState.tsx`) and
