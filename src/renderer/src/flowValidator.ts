// Renderer-side flow validation.
//
// IMPORTANT: do NOT import from '../.../main/flows/validator' here. The main
// validator pulls in the script engine (and with it the whole main-process
// Node graph: fs/path/crypto/worker_threads), which must never reach the
// renderer bundle. This module re-implements the structural, topological, and
// semantic rules of the main validator using only the pure types/schemas from
// main/flows/types (renderer-safe: no Node imports).
//
// The DB-dependent "module script exists" check (MODULE_NOT_FOUND) deliberately
// lives in the main validator only: it runs server-side at flow save/run time.
// The canvas cannot know script existence without a round-trip, and the
// pre-existing renderer behavior never had that check either.
import { FlowNodeSchema } from '../../main/flows/types';
import type { FlowDocument, FlowValidationError, FlowNode, FlowEdge } from '../../main/flows/types';
...
export function validateFlow(flow: FlowDocument): { valid: boolean; errors: FlowValidationError[] } {
  ...
  // 5b. Module node structural validation (existence of the referenced script
  // is enforced server-side at save/run time by the main validator).
  for (const node of flow.nodes) {
