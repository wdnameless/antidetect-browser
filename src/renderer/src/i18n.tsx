...
const RU: Record<string, string> = {
  ...
  'Headless — launch without a window': 'Headless — запуск без окна',
  'For agent and automation profiles. A profile you drive by hand should stay headed.': 'Для агентских и автоматизированных профилей. Профиль, которым вы управляете вручную, лучше оставить с окном.',
  ...
  'Not detected yet': 'Ещё не определено',
  'Not checked yet': 'Ещё не проверен',
  'Profile settings': 'Настройки профиля',
  'Proxy check failed — no exit location': 'Проверка прокси не удалась — нет геолокации',
  'Auto-detect Geo': 'Определить ГЕО',
  'Auto-detecting geo…': 'Автоопределение ГЕО…',
  'Stop Auto-detect': 'Остановить определение',
  'Geo detected': 'ГЕО определено',
  'Detecting…': 'Определение…',
  'Auto-detecting geo in background': 'Фоновое автоопределение ГЕО',
  ...
  'Choose which events send alerts to Telegram:': 'Выберите, какие события отправлять в Telegram:',
  'Profile created': 'Профиль создан',
  'Profile deleted': 'Профиль удален',
  'Task group finished': 'Группа задач завершена',
  'Agent activity': 'Действия агента',
  'High frequency (many alerts per minute while an agent is active)': 'Высокая частота (множество уведомлений в минуту при работе агента)',
  'In-App Notifications': 'Уведомления в приложении',
  'Show agent activity toasts': 'Показывать всплывающие уведомления о действиях агента',
  'Silent popups shown in the corner of the app. Disabling them does not affect Telegram.': 'Бесшумные всплывающие окна в углу приложения. Их отключение не влияет на Telegram.',
  ...
  // Google Drive One-Click Sync & Chromium Mirror
  'Sync all your browser profiles, proxies, tags, notes, vault credentials, scripts, and settings automatically to your personal Google Drive.': 'Автоматическая синхронизация профилей браузера, прокси, тегов, заметок, учетных данных хранилища, скриптов и настроек в ваш личный Google Drive.',
  'Advanced: Custom Google OAuth Credentials': 'Дополнительно: Пользовательские учетные данные Google OAuth',
  'Fallback for development builds or custom Google Cloud projects without a preconfigured client.': 'Резервный вариант для сборок разработки или собственных проектов Google Cloud без встроенного клиента.',
  'Set Encryption Passphrase': 'Установка парольной фразы шифрования',
  'Your sync data is encrypted client-side with AES-256-GCM before being sent to Google Drive. Google never sees your passwords, cookies, or profile data.': 'Данные синхронизации шифруются на клиенте с помощью AES-256-GCM перед отправкой в Google Drive. Google никогда не видит ваши пароли, куки или данные профилей.',
  'Important: Non-Recoverable Passphrase': 'Важно: Парольная фраза не подлежит восстановлению',
  'The passphrase is not stored anywhere. It is required on every machine to decrypt your data. If you forget this passphrase, your Google Drive backup cannot be restored and data will be permanently lost.': 'Парольная фраза нигде не сохраняется. Она потребуется на каждом устройстве для расшифровки ваших данных. Если вы забудете эту парольную фразу, резервную копию с Google Drive будет невозможно восстановить, и данные будут безвозвратно утеряны.',
  'Passphrase (minimum 8 characters)': 'Парольная фраза (минимум 8 символов)',
  ...
  'Passphrase must be at least 8 characters long': 'Парольная фраза должна содержать не менее 8 символов',
  'Passphrases do not match': 'Парольные фразы не совпадают',
  'Confirm & Connect': 'Подтвердить и подключить',
  'Connecting to Google Drive...': 'Подключение к Google Drive...',
  'Please wait while your connection is established. If a browser window opened, follow the instructions to grant access.': 'Пожалуйста, подождите, пока устанавливается соединение. Если открылось окно браузера, следуйте инструкциям для предоставления доступа.',
  'Connection failed': 'Ошибка подключения',
  'Google Drive connected successfully': 'Google Drive успешно подключен',
  'Google Drive sync unlocked for this session': 'Синхронизация Google Drive разблокирована для этого сеанса',
  'Unlock Sync': 'Разблокировать синхронизацию',
  'Enter your passphrase to unlock synchronization for this session:': 'Введите парольную фразу для разблокировки синхронизации на этот сеанс:',
  'Incorrect passphrase': 'Неверная парольная фраза',
  ...
  'Last sync error:': 'Ошибка последней синхронизации:',
  'Automatic Sync': 'Автоматическая синхронизация',
  'Active (syncs on data change, launch, and exit)': 'Включена (синхронизация при изменениях, запуске и выходе)',
  'Connected Account': 'Подключенный аккаунт',
  'Last Synced': 'Последняя синхронизация',
  'Drive Folder': 'Папка Google Drive',
  'Just now': 'Только что',
  'seconds ago': 'сек. назад',
  'minutes ago': 'мин. назад',
  'hours ago': 'ч. назад',
  'days ago': 'дн. назад',
  'pending remote updates waiting to be pulled.': 'ожидающих удаленных обновлений.',
  'Full Chromium Directory Mirror (Experimental)': 'Полное зеркалирование директорий Chromium (экспериментально)',
  'Optional full backup of profile directories in addition to standard portable sync': 'Дополнительное полное резервное копирование папок профилей в дополнение к стандартной переносимой синхронизации',
  'Disabled (Default)': 'Отключено (по умолчанию)',
  'Standard Portable Sync (Active)': 'Стандартная переносимая синхронизация (активна)',
  'Database, profiles, proxies, fingerprints, groups, tags, notes, vault credentials, scripts, settings, and session cookies.': 'База данных, профили, прокси, отпечатки, группы, теги, заметки, хранилище, скрипты, настройки и сессионные куки.',
  'Chromium Directory Mirror': 'Зеркалирование директорий Chromium',
  'Full profile directories including HTTP/code caches and internal runtime files.': 'Полные директории профилей, включая HTTP/код кэши и внутренние файлы.',
  'Note: Chromium caches (~400 MB) are automatically regenerated on the target machine anyway and are not needed to restore profiles.': 'Примечание: Кэши Chromium (~400 МБ) в любом случае автоматически пересоздаются на целевом ПК и не требуются для восстановления профилей.',
  'Warning: High Storage & Machine Binding Restrictions': 'Внимание: Большой объем хранилища и привязка к устройству',
  'Full Chromium mirroring is significantly slower (~795 MB) and consumes cloud quota. Furthermore, raw Chromium cookies are encrypted via Windows DPAPI (tied to the local machine), and Device Bound Sessions are machine-bound, so cookies from Chromium directories may still not transfer to another computer. Standard portable sync (~304 KB) already transfers active sessions safely.': 'Полное зеркалирование Chromium значительно медленнее (~795 МБ) и расходует квоту диска. Кроме того, исходные куки Chromium зашифрованы с помощью Windows DPAPI (привязаны к локальной машине), а Device Bound Sessions привязаны к устройству, поэтому куки из каталогов Chromium все равно могут не открыться на другом компьютере. Стандартная переносимая синхронизация (~304 КБ) уже безопасно переносит активные сессии.',
  'Run Chromium Mirror Backup Now': 'Запустить резервное копирование зеркала Chromium',
  'Full Chromium mirror enabled': 'Полное зеркалирование Chromium включено',
  'Full Chromium mirror disabled': 'Полное зеркалирование Chromium отключено',
  'Failed to update mirror setting': 'Не удалось обновить настройку зеркалирования',
  'Chromium mirror backup completed:': 'Резервное копирование зеркала Chromium завершено:',
  'Mirror backup failed': 'Ошибка резервного копирования зеркала',
  'Sync in progress...': 'Выполняется синхронизация...',
  'Not Connected': 'Не подключено',
  'Connecting…': 'Подключение…',
  'Start Device Authorization': 'Запустить авторизацию устройства',
  ...
  'email.liveReadFailed': 'Не удалось прочитать почтовый ящик',
  'email.inboxUnavailable': 'Ящик не удалось прочитать — причина в сообщении выше',
  ...
  'Copied': 'Скопировано',
  'MCP': 'MCP',
  'Enabled — {n} tools': 'Включён — {n} инструментов',
  'Enabled': 'Включён',
  ...
  'Active (per-seed)': 'Активен (по seed)',
  ...
  'Audio': 'Audio',
  'Canvas': 'Canvas',
  'Mobile profiles only': 'Только мобильные профили',
  ...
  // Preflight & Launch Guard UX
  'Blocking': 'Блокирует',
  'BLOCKING': 'БЛОКИРУЕТ',
  'WARNING': 'ПРЕДУПРЕЖДЕНИЕ',
  'blocking': 'блокирует запуск',
  'fixable': 'исправимо',
  'Auto-fix will not unblock launch: ': 'Авто-исправление не разблокирует запуск: ',
  ' blocking check(s) require manual action.': ' блокирующая проверка требует ручных действий.',
  'Launch Blocked by Preflight Guard': 'Запуск заблокирован защитой Preflight Guard',
  'The preflight launch guard stopped this profile because one or more diagnostic checks failed:': 'Защита перед запуском остановила профиль из-за провала диагностических проверок:',
  'Blocking Checks:': 'Блокирующие проверки:',
  'Re-run Checks': 'Проверить снова',
  'Launch without proxy': 'Запустить без прокси',
  'Launch Anyway': 'Запустить всё равно',
  'Warning: Launching without proxy will route traffic through your real IP address. This exposes your identity and defeats the purpose of an antidetect browser.': 'Внимание: запуск без прокси направит трафик через ваш реальный IP-адрес. Это раскрывает вашу личность в сети и противоречит цели использования антидетект-браузера.',
  '⚠️ Real IP will be exposed': '⚠️ Будет использован реальный IP',
  'Proxy Refused Connection (Transport Failure)': 'Прокси отклонил соединение (ошибка транспорта)',
  'NOT GUARD BLOCKED': 'НЕ БЛОКИРОВКА ЗАЩИТЫ',
  'The launch was NOT blocked by Preflight Guard. The proxy server itself timed out or refused the connection during transport probing.': 'Запуск не был заблокирован Preflight Guard. Прокси-сервер не отвечает или сбросил соединение при проверке транспорта.',
  'Transport Error Details:': 'Детали ошибки транспорта:',
...
const EN: Record<string, string> = {
  ...
  'email.liveReadFailed': 'Could not read the mailbox',
  'email.inboxUnavailable': 'Inbox could not be read — the reason is in the message above',
  ...
  'Headless — launch without a window': 'Headless — launch without a window',
  'For agent and automation profiles. A profile you drive by hand should stay headed.': 'For agent and automation profiles. A profile you drive by hand should stay headed.',
  ...
  'Active (per-seed)': 'Active (per-seed)',
  ...
  'Fonts': 'Fonts',
  'Sensors': 'Sensors',
  'Mobile profiles only': 'Mobile profiles only',
  'WebGL / GPU': 'WebGL / GPU',
  ...
  'Choose which events send alerts to Telegram:': 'Choose which events send alerts to Telegram:',
  'Profile started': 'Profile started',
  'Profile stopped': 'Profile stopped',
  'Profile created': 'Profile created',
  'Profile deleted': 'Profile deleted',
  'Task group finished': 'Task group finished',
  'Agent activity': 'Agent activity',
  'High frequency (many alerts per minute while an agent is active)': 'High frequency (many alerts per minute while an agent is active)',
  'In-App Notifications': 'In-App Notifications',
  'Show agent activity toasts': 'Show agent activity toasts',
  ...
  // Google Drive One-Click Sync & Chromium Mirror
  'Sync all your browser profiles, proxies, tags, notes, vault credentials, scripts, and settings automatically to your personal Google Drive.': 'Sync all your browser profiles, proxies, tags, notes, vault credentials, scripts, and settings automatically to your personal Google Drive.',
  'Advanced: Custom Google OAuth Credentials': 'Advanced: Custom Google OAuth Credentials',
  'Fallback for development builds or custom Google Cloud projects without a preconfigured client.': 'Fallback for development builds or custom Google Cloud projects without a preconfigured client.',
  'Set Encryption Passphrase': 'Set Encryption Passphrase',
  'Your sync data is encrypted client-side with AES-256-GCM before being sent to Google Drive. Google never sees your passwords, cookies, or profile data.': 'Your sync data is encrypted client-side with AES-256-GCM before being sent to Google Drive. Google never sees your passwords, cookies, or profile data.',
  'Important: Non-Recoverable Passphrase': 'Important: Non-Recoverable Passphrase',
  'The passphrase is not stored anywhere. It is required on every machine to decrypt your data. If you forget this passphrase, your Google Drive backup cannot be restored and data will be permanently lost.': 'The passphrase is not stored anywhere. It is required on every machine to decrypt your data. If you forget this passphrase, your Google Drive backup cannot be restored and data will be permanently lost.',
  'Passphrase (minimum 8 characters)': 'Passphrase (minimum 8 characters)',
  ...
  'Please wait while your connection is established. If a browser window opened, follow the instructions to grant access.': 'Please wait while your connection is established. If a browser window opened, follow the instructions to grant access.',
  'Connection failed': 'Connection failed',
  'Google Drive connected successfully': 'Google Drive connected successfully',
  'Google Drive sync unlocked for this session': 'Google Drive sync unlocked for this session',
  'Unlock Sync': 'Unlock Sync',
  'Enter your passphrase to unlock synchronization for this session:': 'Enter your passphrase to unlock synchronization for this session:',
  ...
  'Active (syncs on data change, launch, and exit)': 'Active (syncs on data change, launch, and exit)',
  ...
  'hours ago': 'hours ago',
  'days ago': 'days ago',
  'pending remote updates waiting to be pulled.': 'pending remote updates waiting to be pulled.',
  'Full Chromium Directory Mirror (Experimental)': 'Full Chromium Directory Mirror (Experimental)',
  'Optional full backup of profile directories in addition to standard portable sync': 'Optional full backup of profile directories in addition to standard portable sync',
  'Disabled (Default)': 'Disabled (Default)',
  'Standard Portable Sync (Active)': 'Standard Portable Sync (Active)',
  'Database, profiles, proxies, fingerprints, groups, tags, notes, vault credentials, scripts, settings, and session cookies.': 'Database, profiles, proxies, fingerprints, groups, tags, notes, vault credentials, scripts, settings, and session cookies.',
  'Chromium Directory Mirror': 'Chromium Directory Mirror',
  'Full profile directories including HTTP/code caches and internal runtime files.': 'Full profile directories including HTTP/code caches and internal runtime files.',
  'Note: Chromium caches (~400 MB) are automatically regenerated on the target machine anyway and are not needed to restore profiles.': 'Note: Chromium caches (~400 MB) are automatically regenerated on the target machine anyway and are not needed to restore profiles.',
  'Warning: High Storage & Machine Binding Restrictions': 'Warning: High Storage & Machine Binding Restrictions',
  'Full Chromium mirroring is significantly slower (~795 MB) and consumes cloud quota. Furthermore, raw Chromium cookies are encrypted via Windows DPAPI (tied to the local machine), and Device Bound Sessions are machine-bound, so cookies from Chromium directories may still not transfer to another computer. Standard portable sync (~304 KB) already transfers active sessions safely.': 'Full Chromium mirroring is significantly slower (~795 MB) and consumes cloud quota. Furthermore, raw Chromium cookies are encrypted via Windows DPAPI (tied to the local machine), and Device Bound Sessions are machine-bound, so cookies from Chromium directories may still not transfer to another computer. Standard portable sync (~304 KB) already transfers active sessions safely.',
  'Run Chromium Mirror Backup Now': 'Run Chromium Mirror Backup Now',
  'Full Chromium mirror enabled': 'Full Chromium mirror enabled',
  'Full Chromium mirror disabled': 'Full Chromium mirror disabled',
  'Failed to update mirror setting': 'Failed to update mirror setting',
  'Chromium mirror backup completed:': 'Chromium mirror backup completed:',
  'Mirror backup failed': 'Mirror backup failed',
