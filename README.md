...
- **Обновления (Windows)**: проверка и установка идут через подписанный манифест `latest.json`. Артефакт подписывается minisign-ключом; **публичная** половина ключа вшита в `tauri.conf.json`, **приватная** хранится только в CI-секрете и в репозиторий не попадает. Если ключ не настроен, приложение честно сообщает, что автообновления не сконфигурированы для этой сборки, — вместо красной ошибки на пустом месте. Обновляется установленная сборка; single-file portable обновляется вручную (скачать новый файл), потому что у него нет установщика. **Что нужно настроить в репозитории, чтобы это заработало — в [`docs/RELEASE.md`](docs/RELEASE.md).**
