...
## ADR-009: Лицензирование и защита от форков (SUPERSEDES ADR-004 в части «не коммерческий продукт»)

- **Контекст:** ADR-004 фиксировал скоуп «личный инструмент, без команды/облака/биллинга». С тех пор
  в кодовой базе появились команды, облачный синк, Free/Pro-гейтинг и инструмент выпуска лицензий
  (`src/main/licensing/`, `server/`) — то есть продукт уже стал коммерческим. При этом в репозитории
  не было ни одного файла лицензии (`LICENSE`, `TRADEMARK`, `CLA` отсутствовали), а приватный ключ
  подписи Pro-лицензий лежал в публичном `tests/unit/licenseManager.test.ts` — любой клон мог печатать
  рабочие лицензии (проверено исполнением: подпись слитым ключом принималась зашитым публичным ключом).
- **Решение:**
  1. **AGPL-3.0** как лицензия community-издания + **коммерческая двойная лицензия** для тех, кому
     AGPL не подходит (закрытое использование). Выбрана вместо BUSL-1.1, потому что цель — оставить код
     по-настоящему открытым, а запретить нужно ровно одно: закрытый SaaS поверх него (network-клауза AGPL).
     Смена лицензии действует только на будущие коммиты — форков на момент решения 0.
  2. **Ротация ключа**: приватная половина — вне репозитория навсегда; публичная — единый канонический
     файл `resources/license-public-key.pem`, из которого выводятся и Rust (`include_str!`), и TS-константа.
  3. **Нативная проверка в Rust** (`src-tauri/src/license.rs`): вердикт существует в машинном коде, чтобы
     JS-патч перестал быть достаточным.
  4. **Формат лицензии не меняется** (`<b64url payload>.<b64url sig>`, Ed25519): смена зашитого ключа сама
     по себе аннулирует лицензии, подписанные слитым ключом.
- **Последствия:**
  - **Честная граница защиты:** клиентское приложение нельзя защитить от того, кто готов патчить бинарник.
    Достижимая цель — поднять стоимость обхода с двухминутной правки текста до реверс-инжиниринга.
    Увеличение лимитов, сервер активации и device-locking **не** входят в эту волну (решение оператора:
    «Без привязки — только подпись»).
  - **История git переписывается** (202 коммита, 24 из 54 тегов затронуты) — процедура отрепетирована на
    зеркальном клоне; сам force-push выполняет оператор, поскольку он аннулирует существующие клоны.
  - Гейт Free/Pro **не меняется**: локально всё бесплатно, Pro = команды + облачный синк (решение
    оператора «Как сейчас»). Лимиты профилей и прочие ограничения free-тира не вводятся.
  - AGPL-клауза о сетевом использовании означает: сторонний хостинг этого сервиса как публичного
    обязан раскрыть исходники. Это и есть юридический механизм против закрытого форка.
